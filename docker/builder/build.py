...
IMAGE = "origin-hub-ai-registry.cn-shanghai.cr.aliyuncs.com/component/builder"
BUILDER = "/builder"
...
def package(name: str, arch: str, package: bool, with_sanitizer: str, build_type: str, output: str):
    ...
    if package:
        ...
        cmd = f"find . \( -type f -or -type l \) -and ! \( -name '*.debug' -o -name '*.a' -o -name '*.tgz' -o -name '*.deb' -o -name '*.rpm' -o -name '*.apk' \) | tar --transform 's,^\.,{pkg},' -czvf {pkg}.tgz -T -"
        command(cmd, shell=True, cwd=output)

        pkg = f"{name.lower()}-dbg-{version}-{target_arch}"
        cmd = f"find . \( -type f -or -type l \) -and \( -name '*.debug' -o -name LICENSE -o -name NOTICE.txt \) | tar --transform 's,^\.,{pkg},' -czvf {pkg}.tgz -T -"
        command(cmd, shell=True, cwd=output)
